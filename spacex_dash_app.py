...
def get_pie_chart(entered_site, payload_mass):
    if entered_site == 'All Sites':
        fig = px.scatter(data_frame=spacex_df[spacex_df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
         ...
         fig = px.scatter(data_frame=df[df['Payload Mass (kg)'].between(payload_mass[0], payload_mass[1])], 
                     x='Payload Mass (kg)',
                     y='class',
                     color = 'Booster Version Category',
...
1)KSC LC-39A has the largest number of successful launches
2)KSC LC-39A has the highest success rate at 76.9%
3)2000kg to 4000kg has the highest success rate.
4)5000kg-9000kg has the lowest success rate.
